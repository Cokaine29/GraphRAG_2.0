...
<style>
    ...
    [data-testid="stSidebarContent"] .nav-btn > button {
        width: 100% !important;
    ...
    [data-testid="stSidebarContent"] .nav-btn > button:hover {
        background: #ecfdf5 !important;
        color: #059669 !important;
    }
    [data-testid="stSidebarContent"] .nav-btn-active > button {
        background: #10b981 !important;
        color: white !important;
        font-weight: 600 !important;
        box-shadow: 0 2px 8px rgba(16,185,129,0.25) !important;
    }
    [data-testid="stSidebarContent"] .nav-btn-active > button:hover {
...
def render_sidebar():
    with st.sidebar:
        ...
        page_keys = [k for _, _, k in NAV_PAGES]
        current_idx = page_keys.index(st.session_state.page) if st.session_state.page in page_keys else 0

        selected = option_menu(
            menu_title=None,
            options=[f"{icon}  {label}" for icon, label, _ in NAV_PAGES],
            default_index=current_idx,
            styles={
                "container": {"padding": "0!important", "background-color": "transparent"},
                "icon": {"display": "none"}, # We included the emoji in the text already
                "nav-link": {
                    "font-size": "14px", 
                    "text-align": "left", 
                    "margin": "2px 0px", 
        ...
        # ── Query settings (only relevant for chat page) ──
        if st.session_state.page == "chat":
            ...
            st.slider("LLM Temperature", 0.0, 1.0, 0.0, 0.1,
                      help="Higher = more creative, Lower = more deterministic", key="llm_temp")
            st.slider("Retrieval Top-K", 1, 20, 5,
                      help="Number of chunks to retrieve for Local RAG", key="top_k")
            st.divider()
...
if page == "chat":
    ...
    if query := st.chat_input("Ask a highly complex question across your documents..."):
        ...
        with st.chat_message("assistant"):
            engine = load_engine()
            with st.status("Synthesizing response...", expanded=True) as status_box:
                ...
                try:
                    engine.llm.temperature = st.session_state.get("llm_temp", 0.0)
                    engine.vector_store.top_k = st.session_state.get("top_k", 5)

                    force_val = st.session_state.get("force_route", "🤖 Auto")
                    force = None if force_val.startswith("🤖") else force_val.split()[1]
